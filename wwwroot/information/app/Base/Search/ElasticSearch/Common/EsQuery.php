...
class EsQuery {
    ...
    public function __construct($index, $type = '') {
        Log::info('crm项目老检索被调用:检索名和类型为'.$index.'-'.$type);
        $this->index = $index;
        $this->type = $type;
...
//        $this->boolQuery = $qb->query()->bool();
    ...
    public function where($column, $operator, $value = null, $boolean = 'and') {
        ...
        if($operator == '='){
            $subQuery = $qb->query()->term([$column => $value]);
        }else if($operator == '<'){
            $subQuery = $qb->query()->range($column, ['lt'=>$value]);
        }else if($operator == '>'){
            $subQuery = $qb->query()->range($column, ['gt'=>$value]);
        }else if($operator == '<='){
            $subQuery = $qb->query()->range($column, ['lte'=>$value]);
        }else if($operator == '>='){
            $subQuery = $qb->query()->range($column, ['gte'=>$value]);
        }else if($operator == '<>' || $operator == '!='){
            $subQuery = $qb->query()->term([$column => $value]);
        }else if($operator == 'like'){
            $subQuery = $qb->query()->match($column, ["query"=>$value, "type" => "phrase_prefix"]);
        }
     ...
     *
     * */
    public function wherePath($column,$value,$path=null){
        $qb = new QueryBuilder();
        $nested =$qb->query()->nested();
        $nested->setPath($path);
        $nested->setQuery($qb->query()->term([$column => $value]));
        $this->conds[] = [
            'type'=>'must',
            'query'=>$nested
    ...
    public function whereIn($column, $values) {
        ...
        if($count > 1000){
            ...
            while($values) {
                ...
                $boolQuery->addShould($qb->query()->terms($column, $tmpArr));
            }
            $this->conds[] = [
                'type'=>'must',
                'query'=>$boolQuery
            ...
            $this->conds[] = [
                'type'=>'must',
                'query'=>$qb->query()->terms($column, $values)
            ];
        }
        return $this;
    }
    ...
    public function whereBetween($column, $from, $to) {
        ...
        $this->conds[] = [
            'type'=>'must',
            'query'=>$qb->query()->range($column, ['gte' => $from, 'lte'=>$to])
        ];
        return $this;
    }
    ...
    public function join($type, $column, $value){
        $qb = new QueryBuilder();
        $onQuery = $qb->query()->term([$column => $value]);
        $this->conds[] = [
            'type'=>'must',
    ...
    public function search($start = 0, $pageSize = 10) {
        ...
        //es深分页性能问题优化
        if($start > 200000) {
            $tempResultSet = $this->search->search();
            $allCount = $tempResultSet->getTotalHits();
            if($allCount / 2 < $start) {
                $start = $allCount - $start - $pageSize;
